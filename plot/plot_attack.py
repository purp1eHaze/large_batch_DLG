...
def plot_img(X, Y, label, xlabel, ylabel):
    ...
    l1 = ax1.plot(X, Y[0], color=color_list[0], label=label[0], linewidth=4, marker=marker[0], markersize=16,
                        markerfacecolor= color_list[0],  markeredgecolor='black', linestyle=linestyle[0])

    l2 = ax1.plot(X, Y[1], color= 'royalblue', label=label[1], linewidth=4, marker=marker[1], markersize=20,
                  markerfacecolor= 'royalblue',  markeredgecolor='black', linestyle=linestyle[1])

    l3 = ax1.plot(X, Y[2], color=color_list[2], label=label[2], linewidth=4, marker=marker[2], markersize=16,
                  markerfacecolor= color_list[2],  markeredgecolor='black', linestyle=linestyle[2])
   
    l4 = ax1.plot(X, Y[3], color="c", label=label[3], linewidth=4, marker=marker[3], markersize=16,
                  markerfacecolor= "c",  markeredgecolor='black', linestyle=linestyle[3])

    l5 = ax1.plot(X, Y[4], color="green", label=label[4], linewidth=4, marker=marker[4], markersize=16,
                  markerfacecolor= "green",  markeredgecolor='black', linestyle=linestyle[3])

    l6 = ax1.plot(X, Y[5], color="orange", label=label[5], linewidth=4, marker=marker[5], markersize=16,
                  markerfacecolor= "orange",  markeredgecolor='black', linestyle=linestyle[3])
