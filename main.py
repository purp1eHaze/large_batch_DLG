...
if __name__ == '__main__':
            ...
            for i in range(len(model_time)):
                ...
                def closure():
                    ...
                    if config['cost_fn'] == "sim":
                        grad_diff = reconstruction_costs(dummy_dy_dx, original_dy_dx[i], cost_fn="sim", indices="def", weights="equal")
    ...
    plt.savefig('images/' + args.model + '_' + args.dataset + '/' + 'label_' + args.avg_type + 'bs_' + str(
        args.bs) + 'attack_iter' + str(
        args.attack_iters) + 'start_' + str(args.start_attack_iters) + "_optim_" +args.optim + "_mode_" +args.mode + "_cost_fn_" +args.cost_fn + '.png')
    plt.close()
